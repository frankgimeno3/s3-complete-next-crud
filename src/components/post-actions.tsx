...
export default function PostActions({ onLike, onComment, onRepost, onShare, onDelete }: Props) {
  return (
    <div className="flex -ml-2">
      <div
        className="hover:cursor-pointer hover:bg-neutral-200 dark:hover:bg-neutral-800 flex justify-center items-center h-9 w-9 rounded-full transition-all fill-none dark:stroke-white stroke-black relative -top-px"
        onClick={() => onLike()}
      >
        <svg
      ...
      <div
        onClick={() => onComment()}
        className="hover:cursor-pointer hover:bg-neutral-200 dark:hover:bg-neutral-800 flex justify-center items-center h-9 w-9 rounded-full transition-all fill-none dark:stroke-white stroke-black"
      >
      ...
      <div
        onClick={() => onShare()}
        className="hover:cursor-pointer hover:bg-neutral-200 dark:hover:bg-neutral-800 flex justify-center items-center h-9 w-9 rounded-full transition-all fill-none dark:stroke-white stroke-black relative -top-[1.5px]"
      >
        <svg
      ...
      <div
        onClick={() => onDelete()}
        className="hover:cursor-pointer hover:bg-neutral-200 dark:hover:bg-neutral-800 flex justify-center items-center h-9 w-9 rounded-full transition-all dark:fill-white fill-black relative -top-[1.5px]"
      >
        <svg
          className="w-6 h-6 stroke-2 relative"
          aria-label="Comment"
          role="img"
          viewBox="0 0 24 24"
        >
      ...
      {/* <div
        ...
        className="hover:cursor-pointer hover:bg-neutral-200 dark:hover:bg-neutral-800 flex justify-center items-center h-9 w-9 rounded-full transition-all dark:fill-white fill-black relative -top-[1.5px]"
      >
        <div className="relative w-6 h-6 stroke-2 top-[2px]">
          <Image src="/trash.jpg" fill={true} alt="trash" />
        </div>
      </div> */}
    </div>
  )
